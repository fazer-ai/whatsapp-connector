...
- `presence.set` takes effect when WhatsApp says so, not when the reply comes back: the
  node is written and acknowledged locally. A `chat.presence` sent in the same breath as
  the `available` before it does not render on the other phone; measured, one sent about
  fifteen seconds later does. An account marks itself available once when its session
  opens, so this only bites a client that does both at once.
