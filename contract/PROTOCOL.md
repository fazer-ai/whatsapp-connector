...
**A command with neither field still gets an answer, and the bound under it is not this connector's.** Every request this connector puts on the wire carries the WhatsApp library's own ceiling, seventy five seconds, and that covers a message send and an information query alike; several paths are held to something tighter that this connector chooses. Two do run without one, and they are named below rather than rounded off, because a client that believes every command is bounded will not write the timeout handling it needs. So a client that names no ceiling is not asking for an unbounded command, it is accepting the connector's. What comes back when that bound runs out is `timeout`, which is the same word a caller's own `max_runtime_ms` produces and means the same thing: nobody here knows the outcome and nothing afterwards will. **A client may resend such a command, and for anything that changes something the identifier is not what makes that safe.** The connector records only what succeeded, so a command answered `timeout` has left no record at all, and a resend under the same `idempotency_key` finds nothing to answer from and runs the work again. What the identifier buys is narrower and still worth having: a frame redelivered after the first attempt *succeeded* is answered from the record instead of being run twice. For a send that is enough on its own, because the resend carries the `message_id` the first attempt used and every client downstream discards a repeat of an id it already has. For everything else it is not: a `group.invite.get` that rotates the link, a participant added or removed, a name set, may already have happened, and resending does it again. A client that cannot afford that reads the state back before resending rather than resending blind.

**Two paths are not bounded, and a client that names no ceiling of its own inherits both.** The first is a node already being written to the socket: that write is not interruptible by any ceiling, this connector's or the caller's, so a command waiting behind one can outlast every number above. The second is a message send that was interrupted by the connection dropping and is retried once the connection is back: that retry waits for its answer with no timer of its own, so it ends when the answer arrives or when the session does. Neither is this connector's to fix from here, and the two do not have the same remedy. `max_runtime_ms` reaches the second: the retry watches the context it was given, so a ceiling ends it and a `timeout` comes back. It does **not** reach the first, and a client should not set it expecting one to. The lock is taken before the context is looked at and the connector carries a command out on the goroutine that is waiting there, so neither ceiling runs and no reply is produced until the write ahead finishes. **A client that cannot wait indefinitely needs its own timeout on the reply**, not a field on the command: this connector will answer, and what no field here can promise is when. A `timeout` from either means the wait was given up on, not that WhatsApp answered, and the resend obligation above is what covers it.
