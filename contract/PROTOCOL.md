...
**A client may resend such a command, and for anything that changes something the identifier stops the work being repeated rather than saying what happened.** The connector writes down that a command is about to run and replaces that with what it did, so there are three things it can say about one: that it succeeded, that it ran with nobody able to say how it ended, or that it never happened at all. A resend under the same `idempotency_key` is answered from the first two and carries the work out only on the third. So a command answered `timeout` is answered `timeout` again rather than run a second time, and a command the connector is certain never reached WhatsApp -- the refusals it makes before writing anything -- leaves no record, so the resend does the whole thing. What the identifier buys is therefore two things: a frame redelivered after the first attempt *succeeded* is answered from the record instead of being run twice, and one redelivered after an attempt nobody can speak for is refused instead of being run blind. For a send that second half is belt and braces, because the resend carries the `message_id` the first attempt used and every client downstream discards a repeat of an id it already has. For everything else it is the whole of the cover: a `group.invite.get` that rotates the link, a participant added or removed, a name set, may already have happened, and no answer from here will ever say which. A client that cannot afford that reads the state back before resending rather than resending blind.
