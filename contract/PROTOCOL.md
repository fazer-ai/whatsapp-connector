...
**A send now carries a ceiling of this connector's, and what is left unbounded is a different shape.** The retry that used to wait with no timer of its own is bounded here, so a `message.send` whose caller named nothing still ends. What no ceiling reaches is a wait that does not look at the context at all, and there are two of them on a send's way out: the node already being written to the socket, and the lock the library takes to keep one send per connection at a time. A command held by either does not come back when a ceiling runs out; the ceiling ends the commands queued behind it, once the held call finally returns on its own. So `max_runtime_ms` is worth setting, and it is worth knowing what it buys: it bounds every wait that watches the context, which is all of them but those two. **A client that cannot wait indefinitely needs its own timeout on the reply**, not a field on the command: this connector will answer, and what no field here can promise is when.
