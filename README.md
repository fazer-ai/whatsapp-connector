...
> [!IMPORTANT]
> **Status: M2 is in both ways, and M3 all but the calls.** A session pairs with a real WhatsApp account, resumes
...
> connect decides whether group chats come with them, and a group is created, read, renamed, given a description, a photo and its settings, left, and its participants and join requests administered; a contact is checked for an account, asked for its profile and its picture. An inbound image, video, audio,
...
| **M3** | Groups and contacts are in, calls are not. A group is created, read, listed and left, renamed, given a description, a photo and its settings (`announce`, `locked`, `join_approval`, `member_add_mode`), and its participants added, removed, promoted and demoted; its invite link is served and its join requests are listed and answered. What changes about a group while the connection is up is published rather than waiting for the next reconnect, and `groups` on the connect is remembered, so an account resumed by another instance comes back with the group traffic its client asked for. A contact is checked for an account, asked for its profile and its picture, and resolved to the address the wire uses. **Calls** are shaped in the contract -- `call.offer`, `call.terminate` and `call.reject` -- and nothing behind them: a connect carrying `calls.auto_reject` is refused with `unsupported`, because a build that took the flag and never rejected a call would leave the client waiting for a refusal that was never coming |
