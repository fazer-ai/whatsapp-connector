...
func TestRememberingACommandTwiceKeepsTheFirstAnswer(t *testing.T) {
	...
	result, _, _, err := ledger.Recall(ctx, "s1", "msg:3EB0")
	if err != nil {
		t.Fatalf("Recall: %v", err)
	}
...
func TestARecordBelongsToOneSession(t *testing.T) {
	...
	if _, found, _, err := ledger.Recall(ctx, "s2", "msg:3EB0"); err != nil || found {
