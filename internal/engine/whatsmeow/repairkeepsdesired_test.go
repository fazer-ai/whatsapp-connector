...
func TestARepairedDeviceKeepsWhatTheClientAsked(t *testing.T) {
	...
	for _, pairing := range []string{"resume", "qr"} {
		t.Run(pairing, func(t *testing.T) {
			t.Parallel()
			session, container := newTestSession(t, "5511999990001")
			ctx := t.Context()

			// Seeded, and that is the point rather than a shortcut: what is under test is
			// not who writes the row -- the session layer does, and its own test covers
			// that -- but what is left of it after a connect that rebuilds the device.
			if err := session.store.PutDesiredConnected(ctx, store.Wants{Groups: true}); err != nil {
				t.Fatalf("PutDesiredConnected: %v", err)
			}
			session.markStale()

			// The error is not the assertion. A resume with nothing to resume is refused
			// and a qr connect gets as far as it can without a socket; either way the
			// repair has already run by the time `Connect` returns.
			_ = session.Connect(ctx, engine.ConnectRequest{Pairing: pairing, Groups: true})

			// Read off the table rather than through a store helper, the way
			// `logout_unanswered_test.go` does: the sid is interpolated because
			// `container.DB()` is the raw handle and the placeholder differs per dialect,
			// and it is `"sid-" + t.Name()`, not anything a caller supplies.
			// The row being gone and the row saying something else are the same failure
			// here -- both mean the repair took the request with it -- so they get the
			// same message rather than a bare `sql: no rows in result set`, which names
			// the query and not the defect.
			var wanted string
