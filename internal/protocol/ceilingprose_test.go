...
func TestTheContractSaysWhatBoundsACommandWithNoCeilingOfItsOwn(t *testing.T) {
	...
	for _, clause := range []struct {
		...
		{"resending under the first identifier", "the identifier the first attempt used",
			"a resend under a fresh id runs the side effect twice"},
		{"why the identifier matters", "records only what succeeded",
			"it is the reason the obligation is the client's and not ours"},
